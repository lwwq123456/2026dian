...
class ParallelGDN(nn.Module):
    def __init__(self, d_model):
        ...
        self.norm = ZeroCenteredRMSNorm(d_model)
    ...
    def forward(self, x):
        ...
        device = x.device
        q = F.normalize(F.silu(self.q_proj(x)), p=2, dim=-1)
        k = F.normalize(F.silu(self.k_proj(x)), p=2, dim=-1)
        ...
        log_alpha = torch.log(alpha_squeezed + 1e-8)
        cum_log_alpha = torch.cumsum(log_alpha, dim=1)
        Gamma = torch.exp(cum_log_alpha.unsqueeze(2) - cum_log_alpha.unsqueeze(1)) 
        Gamma = torch.tril(Gamma) 

        K_K_T = torch.bmm(k, k.transpose(1, 2))
        M = torch.tril(beta * (Gamma * K_K_T), diagonal=-1) 
        rhs = beta * v

        I = torch.eye(seq_len, device=device).unsqueeze(0).expand(batch, -1, -1)
        U_bar = torch.linalg.solve_triangular(I + M, rhs, upper=False)

        attn = torch.tril(Gamma * torch.bmm(q, k.transpose(1, 2)))
        gdn_out = torch.bmm(attn, U_bar)
        
        gdn_out = self.norm(gdn_out)
        out_gate = F.silu(self.out_gate_proj(x))
...
class GDNVision(nn.Module):
    def __init__(self, d_model=128, num_classes=10, num_layers=4):
        ...
        self.final_norm = ZeroCenteredRMSNorm(d_model)
...
def train_and_evaluate():
    ...
    print(f"⚡ 启动任务 3.3 (Parallel Optimized) | 设备: {device}")
    ...
    plt.title('Task 3.3: Parallel Optimized Metrics', fontsize=14, fontweight='bold')
    fig.tight_layout()
    plt.savefig('task3_3_optimized.png')
